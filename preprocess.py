...
# [1.0] add mail id for every record

sid_mid = si[['sid', 'mid']]
# extended_user_shop_behavior (mail_id added)
eusb = pd.merge(usb, sid_mid, on='sid')

# eusb.to_csv('i:tianchi/eusb.csv', index=None)

# [2.0] generate subset, one subset for one mail, sorted by time
# extended_user_shop_behavior
# eusb = pd.read_csv('i:tianchi/eusb.csv', header=0, low_memory=False)

mail_group = eusb.groupby('mid')

i = 0
for k, v in mail_group:
    i += 1
    sorted_v = v.sort_values(by=['time'])
    sorted_v.to_csv('./subset/mail%d.csv' % i, index=None)
